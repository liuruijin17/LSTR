...
def kp_detection(db, nnet, result_dir, debug=False, evaluator=None, batch_size=1,
    ...
    for epid in tqdm(range(0, test_epoch), ncols=67, desc="Fitting curves"):
        ...
        for bind in range(len(ids_in_batch)):
            ...
            # raw_image is only used for vis
            raw_img = image.copy()
...
def testing(db, nnet, result_dir, debug=False, evaluator=None, batch_size=1,
    ...
    return globals()[system_configs.sampling_function](db, nnet, result_dir, debug=debug, evaluator=evaluator,
                                                       batch_size=batch_size, isEncAttn=debugEnc, isDecAttn=debugDec)